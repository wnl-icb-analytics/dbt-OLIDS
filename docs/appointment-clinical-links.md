...
record types. `clinical_record_id` is the detail table's `id`, including the
minted observation IDs for added allergies and referrals. Clinical codes,
labels, dates and results remain available in those detail tables.
...
Only the narrow relationship is materialised. The conformed definition is a
view, so the join is not stored twice. Clustering by appointment ID supports
clinical drill-down. There is no new index of all clinical history.
...
Both models and their four tests compile. Independent architecture review found
...
The total is 71,780,736 links, with no missing values in the six output columns.
