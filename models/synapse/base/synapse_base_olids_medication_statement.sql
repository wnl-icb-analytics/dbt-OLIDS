...
LEFT JOIN {{ ref('synapse_int_enriched_concept_map') }} concept_map
    ON src.medication_statement_source_concept_id = concept_map.source_concept_id
LEFT JOIN {{ ref('synapse_int_enriched_concept_map') }} auth_concept_map
    ON src.authorisation_type_source_concept_id = auth_concept_map.source_concept_id
LEFT JOIN {{ ref('synapse_int_enriched_concept_map') }} date_precision_map
