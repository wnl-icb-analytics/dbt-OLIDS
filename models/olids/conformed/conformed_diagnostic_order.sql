...
/*
Conformed DIAGNOSTIC_ORDER view.
...
SELECT
    ...
    src.result_value,
    src.result_measurement_units_source_concept_id,
    src.result_date,
    src.result_text,
    src.is_problem,
    src.is_review,
...
FROM {{ ref('landing_diagnostic_order') }} src
...
LEFT JOIN {{ ref('int_enriched_concept_map') }} concept_map
    ON src.diagnostic_order_source_concept_id = concept_map.source_concept_id
